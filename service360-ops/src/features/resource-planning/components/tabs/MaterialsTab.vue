<template>
  <div>
    <ExistingDataBlock :existingRecords="existingRecords" dataType="materials" />

    <div class="new-info-content">
      <div v-for="(material, index) in records" :key="index" class="resource-record-block">
...
const createNewObject = () => ({ material: null, unit: null, volume: null });
...
const save = async () => {
  const validRecords = records.value.filter(m => m.material && m.unit && m.volume != null && m.volume > 0);
...
@media (max-width: 768px) {
  .form-line-materials { grid-template-columns: 1fr; }
