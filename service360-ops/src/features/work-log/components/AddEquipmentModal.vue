  ...
  <div v-if="isOpen" class="modal-overlay" @click="handleOverlayClick">
    <div class="modal-container" @click.stop>
      <div class="modal-header">
        <h3 class="modal-title">Добавить технику</h3>
      ...
      <div class="modal-body">
        ...
        <div v-else>
          ...
          <div class="form-group">
            <label>Техника <span class="required">*</span></label>
            <AppDropdown
              id="equipment-select"
              v-model="selectedEquipments"
              :options="equipmentOptions"
              placeholder="Выберите одну или несколько единиц техники"
              :disabled="!selectedLocation"
              multiple
            />
            <div v-if="selectedLocation && equipmentOptions.length === 0" class="empty-equipment-text">
              Нет доступной техники для выбранного участка
            </div>
          </div>
        </div>
      </div>
...
const locationOptions = computed(() => {
  ...
  allEquipments.value.forEach(equipment => {
    if (equipment.objLocationClsSection && equipment.nameLocationClsSection) {
      uniqueLocations.set(equipment.objLocationClsSection, {
        value: equipment.objLocationClsSection,
        label: equipment.nameLocationClsSection
      });
    }
  });
...
const filteredEquipments = computed(() => {
  ...
  return allEquipments.value.filter(
    equipment => equipment.objLocationClsSection === locationValue
  );
});
...
const loadEquipments = async () => {
  ...
  if (props.pvTypEquipment === null) return;
  try {
    const records = await loadResourceByTyp(props.pvTypEquipment, "equipment", "Prop_Equipment");
    allEquipments.value = records.map(record => ({
      id: record.id,
      cls: record.cls,
      pv: record.pv,
      name: record.name,
    ...
    notificationStore.showNotification('Ошибка при загрузке списка техники', 'error');
...
const saveEquipments = () => {
  ...
  const selectedEquipmentData = allEquipments.value.filter(equipment =>
    selectedEquipments.value.includes(equipment.id)
  );

  emit('save', {
    location: selectedLocation.value.value,
    equipments: selectedEquipmentData
  });
};
